...
class NetworkBasic(nn.Module):
    def __init__(self, number_categories: int = 8, emb_size: int = 10) -> None:
        ...
        self.tconv1 = nn.ConvTranspose2d(
            64 + self.emb_size, 32, kernel_size=4, stride=2, padding=1
        )
...
class NetworkAdvanced(nn.Module):
    def __init__(
        ...
        # Dilation.
        self.dilat1 = nn.Conv2d(
            256, 256, kernel_size=4, stride=1, padding=3, dilation=2
        )
        self.dilat1_bn = nn.BatchNorm2d(256)
        self.dilat2 = nn.Conv2d(
            256, 256, kernel_size=4, stride=1, padding=3, dilation=2
        )
        ...
        # Decoder.
        self.tconv1 = nn.ConvTranspose2d(
            256 + self.emb_size, 128, kernel_size=4, stride=2, padding=1
        )
    ...
    def forward(
        ...
        gray_embb_out = cat((gray_dilat2, embb_category_out), 1)
