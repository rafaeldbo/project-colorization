...
from torch import optim, nn, save, load
from torch.utils.data import DataLoader

from .dataset import ImageDataset


def train_model(model:nn.Module, model_name:str, images_folder:str, dir:str='./', 
                from_checkpoint:bool=False, checkpoint_period:int=25,
                amount_images:int=4, epochs:int=50, learning_rate:float=0.001, 
                cuda:bool=True, batch_size:int=32, prefetch_factor:int=None, num_workers:int=0
    ) -> None:
    ...
    dataloader = DataLoader(data, batch_size=batch_size, shuffle=True, pin_memory=pin_memory, prefetch_factor=prefetch_factor, num_workers=num_workers)
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(dataloader):
            ...
            print(f'[{epoch+1:2d}, {i + 1:2d}] loss: {loss.item()/2000:.3f}')
        running_losses.append(epoch_running_loss)
        
        # Saving the model every checkpoint_period epochs.
        if epoch % checkpoint_period == 0 and epoch > epochs:
            save({
                "epoch": epoch,
